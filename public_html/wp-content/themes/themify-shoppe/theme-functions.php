...
function themify_theme_menu_icon( $output, $id, $title, $icon ) {
	$output = '<em>'. themify_get_icon( $icon ) . '</em><span class="tooltip">' . $title . '</span>';
...
function themify_add_tile_layout($options,$module){
	if($module->slug==='products' || $module->slug==='post'){
		...
		$bindig = array(
			...
			'list-post' => array(
				'hide' => array('masonry_post ','post_gutter','post_filter'),
			...
			'list-thumb-image' => array(
				'hide' => array( 'content_layout', 'masonry_post', 'post_gutter','post_filter' )
			),
			'grid2-thumb' => array(
				'hide' => array( 'content_layout', 'masonry_post', 'post_gutter' ),
                'show' => array( 'post_filter' )
			)
		);
