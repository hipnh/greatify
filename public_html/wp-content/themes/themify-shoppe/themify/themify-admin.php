...
function themify_page() {
	...
	<?php
	/* Detection of GZ issue in LiteSpeed server
	 */
	if ( themify_get_server() === 'litespeed' ) : ?>
		...
		if ( ! themify_are_plugins_active( wp_list_pluck( $required_plugins, 'path' ) ) ) {
			...
			if (!defined('THEMIFY_UPDATER')) {
			    ...
			    if ( isset( $all_plugins[$updater['path']] ) ) {
			        $output .= sprintf( "<p>(<a href='%s' class='external-link' target='_blank'>%s</a>) %s</p>", wp_nonce_url( admin_url( 'plugins.php?action=activate&plugin=' . $updater['path'] ), 'activate-plugin_' . $updater['path'] ), __('Activate', 'themify'), __('Themify Updater to install other plugins', 'themify'));
			    } else {
...
function themify_recaptcha_setting($data=array()){
    $version=themify_get( 'setting-recaptcha_version','',true );
    $options='';
    for($i=2;$i<4;$i++){
    ...
    $output.='<p><span class="label">' . __( 'ReCaptcha Site Key', 'themify' ) . '</span> <input type="text" class="width10" name="setting-recaptcha_site_key" value="' . esc_attr( themify_get( 'setting-recaptcha_site_key','',true ) ) . '" /></p>';
    $output.='<p><span class="label">' . __( 'ReCaptcha Secret Key', 'themify' ) . '</span> <input type="text" class="width10" name="setting-recaptcha_secret_key" value="' . esc_attr( themify_get( 'setting-recaptcha_secret_key','',true ) ) . '" /><br/><span class="pushlabel"><small>'.sprintf( __( 'Go to <a target="_blank" href="%s">reCAPTCHA Admin Console</a> to create API keys for your domain. A Google account is required.', 'themify' ), 'https://www.google.com/recaptcha/admin' ).'</small></span></p>';;
    return $output;
}
