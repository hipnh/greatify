...
class Themify_Enqueue_Assets {
    ...
    public static $SWIPER_VERSION = '5.3.9';
    ...
    public static function header_meta() {
        ...
        <meta name="keywords" content="Mua quần áo, Đồ unisex, đồ lót, Thời trang nam, Greatify, xu hướng thời gian" />
        <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
        <meta name="author" content="Nhóm 3 - IE204.M22" />
        <meta name="geo.region" content="VN" />
        <meta name="geo.placename" content="Thành phố Thủ Đức" />
        <meta name="geo.position" content="10.869793;106.80309" />
        <meta name="ICBM" content="10.869793, 106.80309" />
        <!-- Open Graph (OG) meta tags are snippets of code that control how URLs are displayed when shared on social media  -->
        <meta property="og:image" content="link logo" />
        <?php
    }
    ...
    public static function wp_head() {
        ...
        if (themify_is_lazyloading() === true) {
            ...
            <style id="tf_lazy_style">
                ...
                .tf_svg_lazy{transition:filter .3s linear!important;<?php if ($blur > 0): ?>filter:blur(<?php echo $blur ?>px);<?php endif; ?>opacity:1;transform:translateZ(0)}
        ...
        if (self::$concateFile !== null) {
            ...
            if ($key !== '') {
                ...
                if ($exist === false) {
                    ...
                    foreach (self::$css as $k => $v) {
                        if ($k !== 'in_footer' && $k !== 'mobile_concate') {
                            ...
                            if (!empty($content)) {
                                ...
                                if($k=== 'themify-customize'){
                                    $customizer_str=PHP_EOL . '/*' . str_replace($replace, '', $v['s']) . '*/' . PHP_EOL .  $content;
                                }else{
                            ...
                            if($k=== 'themify-customize'){
                                $customizer_output = '<link rel="preload" href="' . $v['s'] . '?ver=' . $v['v'] . '" as="style">' . "\n" . '<link id="' . $k . '-css" rel="stylesheet" href="' . $v['s'] . '?ver=' . $v['v'] . '">' . "\n";
                            }else{
                    ...
                    // Append customizer to th end (Must load after all CSS)
                    if(!empty($customizer_output)){
                        $output .= $customizer_output;
                        unset($customizer_output);
                    }
                    if(!empty($customizer_str)){
                        $str .= $customizer_str;
                        unset($customizer_str);
                    }
                    if ($key !== '') {
                        clearstatcache();
    ...
    public static function addPreLoadJs($src, $ver,$importance=false) {
		if($importance===false){
			$importance='low';
		}
        self::addPreLoadMedia(themify_enque($src),'preload','script',$ver,null,$importance);
    }
    
    public static function addPrefetchJs($src, $ver,$importance=false) {
		if($importance===false){
			$importance='low';
		}
        self::addPreLoadMedia(themify_enque($src),'prefetch','script',$ver,null,$importance);
    }

    public static function addPreLoadCss($src, $ver, $m = 'all',$importance=false) {
        self::addPreLoadMedia(themify_enque($src),'preload','style',$ver,$m,$importance);
    }
    
    public static function addPrefetchCss($src, $ver, $m = 'all',$importance=false) {
        self::addPreLoadMedia(themify_enque($src),'prefetch','style',$ver,$m,$importance);
    }

    public static function addPreLoadMedia($src, $rel = 'preload', $type = 'image',$ver='',$m = 'all',$importance=false) {
        if($rel==='prefetch' && isset(self::$preLoadMedia[$src])){
            return;
        }
    ...
    public static function loadMainScript() {
        ...
        if (!wp_script_is('themify-main-script') && empty($_GET['legacy-widget-preview']) /* disable in Block widget preview */
            ...
            if(current_theme_supports('themify-wc-accordion-tabs')){
                $args['js_modules']['wcacc']=themify_enque( $jsUrl . 'wc-accordion-tabs.js' );
            }
